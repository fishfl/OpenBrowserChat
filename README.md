...
OpenBrowserChat 是一款基于 Chrome 的开源浏览器扩展，旨在将任何网页内容轻松添加为上下文，并使用各种主流的大语言模型（LLM）进行问答或对话。并且，它还内置了 **SKILL（技能）管理体系**，让你可以自定义技能指令并快速调用。
...
*(演示：在阅读说明文档时，随时框选多段不同章节的上下文，加入到 AI 对话上下文中，综合进行发问)*

- ⌨️ **全局唤出助手**: 通过快捷键 `Alt+V` 可以随时在当前网页中唤出 AI 助手面板，无需离开当前浏览流程。
- 🚀 **一键添加上下文**: 在网页中选中任何文本，使用快捷键 `Alt+V` （Mac 同理）即可立即将选中的文本捕获为对话上下文，可以跨页签，跳转不丢失。
- 🧠 **广泛的模型支持**: 通过适配通用的 OpenAI 接口格式，完美支持大量主流模型及提供商（也支持自定义 Endpoint ）：
  - OpenAI 
  - Google (Gemini)
  - DeepSeek
  - 月之暗面 (Kimi)
  ...
  - 自定义
- 🛠 **支持文本类 SKILL 技能系统**: 
  - 支持上传 `.md` 格式文件配置技能（带有 YAML Frontmatter 格式的配置）。
  - 在助手输入框中输入 `/` 即可触发技能补全列表；支持通过 `/skill-name` 快速携带特定增强提示词调用 AI（例如去AI味、翻译、结构化输出等）。
...
3. 打开 Chrome (或 Edge/Brave 等基于 Chromium 的浏览器)，在地址栏输入 `chrome://extensions/` 访问扩展程序管理页。
...
3. **在浏览器中加载扩展**
   ...
   - 选中本项目中打包后的整个文件夹所在的路径（带有 `manifest.json` 配置的根目录作为插件目录载入）进行安装。
